...
public class OwnerAddReservation extends JPanel {

	OwnerReservationsPanel parent;
	String customerId;
	JLabel title;
	SQLRestaurant s;
	Vector<String> restaurantOptions;
	Vector<String> customerOptions;
	...
	public OwnerAddReservation(Object parent, String rid) {
		...
		gc.gridx = 1;
		formPanel.add(customerComboBox, gc);
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (selectedDate == null) {
					...
					JOptionPane.showMessageDialog(null,"Oops! Selected day must be in the future. Please call the restaurant directly for same-day bookings.", "Date Missing", JOptionPane.PLAIN_MESSAGE);
						...
						boolean result = s.requestReservationByOwner(selectedDate, selectedTime, restID, partySize, selectedCustomer);
						if (result) {
							String success = "Success! Customer confirmed for " + selectedTime + " on " + formatDate(selectedDate) + ".";
	...
	private void initializeElements() {
		...
		customerOptions = s.getCustomerUsernames();
		
		restaurantComboBox = new JComboBox(restaurantOptions);
		customerComboBox = new JComboBox(customerOptions);

		customerLabel = new JLabel("Choose a customer: ");
